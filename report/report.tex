...
\begin{table}[H]
  ...
  \begin{tabular}{| c | c | c | c | c | c | c | c | c |}
    ...
    \hline
    \multirow{6}{*}{P.D.} & \multirow{2}{*}{$1$} & {\itshape false} & $7$ & $0.0016$ & $\lambda$ & $27.54401$ & $2$ & $78.56617$ \\
    \cline{3-9}
    ...
    \multirow{6}{*}{P.SD.} & \multirow{2}{*}{$1$} & {\itshape false} & $7024$ & $10.1979$ & $\lambda$ & $151.9145$ & $2$ & $0.77504$ \\
    \cline{3-9}
...
\begin{table}[H]
  ...
  \begin{tabular}{| c | c | c | c | c | c | c | c | c |}
    ...
    \cline{3-9}
     & & {\itshape true} & $3536$ & $13.319$ & $\lambda$ & $18.93256$ & $2$ & $277.3673$ \\
    \cline{2-9}
    ...
    \multirow{6}{*}{$33$} & \multirow{2}{*}{$1$} & {\itshape false} & $18860$ & $265.94$ & $\lambda$ & $578.480$ & $2$ & $39.12439$ \\
    \cline{3-9}
    ...
    \multirow{6}{*}{$50$} & \multirow{2}{*}{$1$} & {\itshape false} & $14139$ & $107.0964$ & $\lambda$ & $4440.370$ & $2$ & $20.7995$ \\
    \cline{3-9}
    ...
    \cline{3-9}
     & & {\itshape true} & $11304$ & $76.041$ & $\lambda$ & $32265.3$ & $4$ & $27.2090$ \\ 
    \cline{2-9}   
    ...
    \multirow{6}{*}{$80$} & \multirow{2}{*}{$1$} & {\itshape false} & $4290$ & $18.700$ & $\lambda$ & $34.4769$ & $2$ & $56.6695$ \\
    \cline{3-9}
...
\begin{table}[H]
  ...
  \begin{tabular}{| c | c | c | c | c | c | c | c |}
    ...
    \cline{2-8}
     & {\itshape true} & $2136$ & $.596$ & $\lambda$ & $49.0422$ & $10$ & $20.4259$ \\
    \hline
    \multirow{2}{*}{$2$} & {\itshape false} & $14710$ & $21.287$ & $\lambda$ & $45.1122$ & $3$ & $24.3559$ \\ 
    \cline{2-8}
...
We can notice from the above plots that the guarantee of having an ascent direction is not ensured. Indeed in some cases we notice how the lagrangian trend tends to decrease instead of increasing, and as a consequence the dual gap behavior tends
to increase, which is not what we want. That's why we always keep track of the best dual function value. Also, we see how in other cases, in particular when using deflection, the value tends to stabilize, reaching a local maxima. Regarding the 
$\lambda$/x norm, we see how the decreasing fashion towards the imposed threshold of $10^{-6}$ is not quite smooth in most cases.\\
In the vastly number of tries, the $\lambda$-norm is the stopping condition which terminates the execution. The threshold set on the dual gap works better on low magnitude experiments (with small $n$), while the threshold of \textit{max\_iter} is 
never reached.

\section{Code description}

The code is provided in Julia through package. To try it, simply open the Julia REPL inside the current folder (specifying the \texttt{--project=.} directive) and then \texttt{instatiate}. Then you need only to \texttt{include{src/main.jl}} and 
try values of $n$ and $K$ for yourself.\\
All the code is inside the \texttt{src} folder:
\begin{itemize}
  \item \textit{Utils.jl}: module containing some useful functions;
  \item \textit{JuMPSolution.jl}: module computing the off-the-shelf primal solution;
  \item \textit{ADAGRAD\_Solver.jl}: module implementing the different update rules;
  \item \textit{main.jl}: used to test all the code;
\end{itemize}
In addition, \textit{util.sh} is a clean-up routine for the \textit{logs} and \textit{plots} produced, and \textit{analyzer.py} is used to inspect the collected \textit{.csv} files and get some stats.
