...
\begin{gather*}
  ...
  \end{bmatrix}
\end{gather*}
which can be solved efficiently through backsubstitution.\\
      ...
      \begin{algorithmic}
        \Function{\texttt{ADAGRAD}}{$\eta$, $\delta$, $\epsilon$, $max\_iter$}
          ...
          \For{$t \gets 1$ to $T$}
            ...
            \State $g_t \gets \dfrac{\partial \psi_\lambda(\lambda_{t-1})}{\partial \lambda_{t-1}}$\Comment{Compute subgradient}
            \State $g_{1:t} \gets \left[ g_{1:t-1}\ g_t \right]$\Comment{Store subgradient}
...
% I think the problem of finding $x_t$ can be solved in the following way. Let's start by rewriting the equation update for the variable $x_t$:
% \[
%   x_t = \argmin_{x \in \mathcal{Y}} \{ x^T Q x + q x - \left\langle \lambda_t,x \right\rangle \}
% \]
% So we can identify the projection set, which is the line defined by the vector $x_{t-1}$ with the following condition $\mathcal{Y} = \{ \sum_{i \in I^k} x_i = 1 \}$.\\
% So we have a basically an hyperplane in $\mathbb{R}^n$ whose defining vector $x$ forms $k$ separates unitary simplex.\\
% By normalizing the vector $x$, we can obtain $u$. Now the projection can be rewritten as:
% \[
%   Proj_\mathcal{Y}(\tilde{x}) = (\tilde{x} \cdot u) \cdot u
% \]
% $u$ is our vector $x$ normalized which define the projection line. The value $\tilde{x}$ is the value obtained from the inner computation of the $\argmin$, which is then projected. The product between $\tilde{x}$ and $u$ forms a matrix $A$, which is the "transformation matrix" used 
% to calculate the projected result.
