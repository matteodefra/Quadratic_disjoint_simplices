...
\begin{center}
  \begin{tabular}[H]{| L | L | L | L |}
    ...
    \hline
    50 & \left[13,25,37\right] & 10^{-6} & 100000 \\
    \hline
    100 & \left[25,50,75\right] & 10^{-6} & 100000 \\
    \hline
    250 & \left[70,125,190\right] & 10^{-6} & 100000 \\
    \hline
    % 500 & \left[125,250,375\right] & 10^{-6} & 100000 \\
    % \hline
  \end{tabular}  
\end{center}

The code was executed using Julia \textit{REPL} tool. It has a slowish startup time, so the first execution are "warm-up" execution, but as soon as the entire code is compiled, modules are loaded and the needed memory is computed, 
then the next executions are much much faster. We suggest to do first executions with low magnitude of dimensions ($n=2$ and $K=\left[1,2\right]$) and ignore the results, and then start the experiments with 
...
Another observation regard the subgradient. Being the $\phi$ non differentiable in the general case, the direction is not guaranteed to be ascent/descent. So we keep the best function value, best iteration, best $x$ 
and $\lambda$ to get the suboptimal solution found.
...
\[
  \psi(y) \le \psi(x) + s(y-x) \quad \forall y \in \mathbb{R}^n  
\]
which reordering the term can be written as
\[
  \langle s, y-x \rangle \ge \psi(y) - \psi(x) \quad \forall y \in \mathbb{R}^n  
\]
Now, since we want to compute the subgradient at the point $x = \lambda_{t-1}$ (we will omit $\forall y \in \mathbb{R}^n$ in the next steps)
\[
  \langle s, y - \lambda_{t-1} \rangle \ge \psi(y) - \psi(\lambda_{t-1})   
\]
Substituting $y = \lambda$, calling $v_1 = \psi(\lambda_{t-1})$, and using the function at our disposal, we have to solve the following inequality
\[
  \langle s, \lambda - \lambda_{t-1} \rangle \ge x^T Q x + q x - \lambda x - v_1
\]
...
the quantity $x^T Q x + q x = v_2$, obtaining 
\[
  \langle s, \lambda - \lambda_{t-1} \rangle \ge v_2 - \lambda x - v_1
\]
Either we can solve the above inequality, or better we can apply the following reasoning. The subdifferential is the set containing all the numbers in the 
...
% \end{align*}
% Being everything constant except for the relaxed constraint, we have the subgradient with respect to the set of inequality constraints $h$ 
% \begin{align*}
%   \dfrac{\partial h(\lambda_{t-1})}{\partial \lambda_{t-1}} = \dfrac{\partial}{\partial \lambda_{t-1}} - \left\langle \lambda_{t-1},x \right\rangle
% \end{align*}
