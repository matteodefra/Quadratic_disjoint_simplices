...
step by step, and in the end stored in a \textit{csv} file and used to display a convergence plot (\textit{iterations} vs \textit{Lagrangian value}) and a residual visualization plot of the 
$\lambda$-norm (\textit{iterations} vs $\lambda$\textit{-norms}). All the duality gaps and the resulting value of the Lagrangian functions are saved too in a dictionary and printed at the end 
to identify the best duality gap (among the different update rules) and the function value.\\
